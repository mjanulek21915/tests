...
static void			get_temp_hex(va_list vl, t_type *type)
{
	if (type->is_long == 1)
...
static void			ft_pf_write_hex_2(t_type *type, long long *rst)
{
	if (type->print_none)
		type->len = 0;
	...
	if (type->is_left)
	{
		...
		if (!type->print_none)
			ft_pf_write_put_base(type->data.u_temp, BASE_16, rst);
...
void				ft_pf_write_hex(va_list vl, t_type *type, long long *rst)
{
	if (type->type == 'x')
	{
		get_temp_hex(vl, type);
		if (type->is_precision && type->precision == 0 &&
		type->data.u_temp == 0)
			type->print_none = 1;
